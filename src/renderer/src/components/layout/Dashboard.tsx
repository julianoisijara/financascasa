...
export default function Dashboard({ appData }: Props) {
  ...
  if (appData.years[selectedYear]) {
    for (let m = 1; m <= 12; m++) {
      ...
      if (filterCategoryId !== 'all') mExpenses = mExpenses.filter(e => e.categoryId === filterCategoryId)
  ...
  // Category colors
  const catColors = ['bg-emerald-500', 'bg-blue-500', 'bg-amber-500', 'bg-purple-500', 'bg-rose-500', 'bg-cyan-500', 'bg-orange-500', 'bg-indigo-500']
    ...
    <div className="flex-1 overflow-y-auto bg-background">
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          {period === 'month' && (
            <select
              ...
              className="bg-white/5 border border-border rounded-lg text-xs font-medium text-foreground px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary/50 cursor-pointer"
            >
              {months.map(m => <option key={m} value={m} className="bg-background">{getMonthName(m)}</option>)}
            </select>
          )}
          ...
          <select
            ...
            className="bg-white/5 border border-border rounded-lg text-xs font-medium text-foreground px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary/50 cursor-pointer"
          >
            <option value="all" className="bg-background">Todos os Usuários</option>
            {appData.users.map(u => <option key={u.id} value={u.id} className="bg-background">{u.name}</option>)}
          </select>
          ...
          <select
            ...
            className="bg-white/5 border border-border rounded-lg text-xs font-medium text-foreground px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary/50 cursor-pointer"
          >
            <option value="all" className="bg-background">Todas as Categorias</option>
          ...
          >
            ...
            <option value="shared" className="bg-background">Custo da Casa</option>
            <option value="extra" className="bg-background">⚡ Valor Extra</option>
          </select>

          {(filterUserId !== 'all' || filterCategoryId !== 'all' || filterType !== 'all') && (
            <button
              onClick={() => { setFilterUserId('all'); setFilterCategoryId('all'); setFilterType('all') }}
        ...
        <div className="grid grid-cols-4 gap-4">
          <SummaryCard label="Total Geral" value={formatCurrency(totalGeral)} icon="💰" accent="emerald" />
          <SummaryCard label="Custo da Casa" value={formatCurrency(totalShared)} icon="🏠" accent="blue" />
          <SummaryCard label="Valor Extra" value={formatCurrency(totalExtra)} icon="⚡" accent="amber" />
          ...
          <div className="col-span-2 card p-6 flex flex-col">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Visão geral do balanço em {selectedYear}</p>
              </div>
              <div className="flex flex-wrap justify-end gap-3 text-[9px] font-bold text-muted-foreground uppercase tracking-wider max-w-[50%]">
                {categories.slice(0, 4).map(c => (
                  <span key={c.id} className="flex items-center gap-1.5">
                    <span className={cn("w-2 h-2 rounded-[3px]", getColorForCategory(c.name))}></span>
            ...
            <div className="relative flex-1 min-h-[220px] flex">
              ...
              <div className="relative z-10 flex items-end justify-between w-full pl-14 pr-2 pb-6 pt-2 h-full">
                {monthlyData.map(d => {
                  ...
                  return (
                    <div key={d.month} className="relative flex flex-col items-center group h-full justify-end w-10 hover:z-50">
                        ...
                        <div className="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0 pointer-events-none z-[60] w-48 bg-background border border-border shadow-2xl rounded-xl p-3">
                          ...
                          <div className="space-y-2">
                              ...
                              .map(([catName, amount]) => (
                                <div key={catName} className="flex justify-between items-center text-xs">
                                  <span className="flex items-center gap-1.5 text-muted-foreground truncate max-w-[100px]">
                                    <span className={cn("w-1.5 h-1.5 rounded-full", getColorForCategory(catName))}></span>
                                    {catName}
                                  </span>
                            ...
                            <div className="flex justify-between items-center text-xs pt-1 mt-1 border-t border-border/50">
                              ...
                              <span className="font-bold text-foreground">{formatCurrency(d.total)}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div
                          className={cn(
                            ...
                            isCurrentMonth 
                              ...
                              : d.total === 0 ? "bg-transparent" : "bg-white/[0.02]"
                          )}
                        >
                          {d.total > 0 && Object.entries(d.categories)
                            .sort((a, b) => a[1] - b[1]) // ascending so largest is at the bottom
                            .map(([catName, amount]) => (
                              <div
                                key={catName}
                                className={cn("w-full transition-all duration-300", getColorForCategory(catName))}
                  ...
                  return (
                    <div key={catName}>
                      <div className="flex justify-between text-xs mb-1">
                        ...
                        <span className="text-muted-foreground font-medium ml-2 flex-shrink-0">{pct.toFixed(0)}%</span>
                      </div>
                      <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                        <div
                          className={cn("h-full rounded-full transition-all duration-700", getColorForCategory(catName))}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                      <p className="text-[10px] text-muted-foreground mt-0.5">{formatCurrency(amount)}</p>
                    </div>
                  )
                })}
              </div>
        ...
        <div className="grid grid-cols-2 gap-5">
          {/* Per-user */}
          <div className="card p-5">
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-4">Gastos por Participante</p>
            {userTotals.size === 0 ? (
              <p className="text-xs text-muted-foreground text-center py-8">Sem dados</p>
            ) : (
              <div className="space-y-3">
                      ...
                      <div key={userId} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between text-xs mb-1">
                            ...
                            <span className="text-foreground font-bold ml-2 flex-shrink-0">{formatCurrency(amount)}</span>
                          </div>
          ...
          {/* Top 5 expenses */}
          <div className="card p-5">
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-4">Top 5 Maiores Despesas</p>
            {top5.length === 0 ? (
              <p className="text-xs text-muted-foreground text-center py-8">Sem dados</p>
            ) : (
                ...
                {top5.map((e, idx) => {
                  ...
                  return (
                    <div key={e.id} className={cn(
                      ...
                      isExtra ? 'bg-amber-500/5 border-amber-500/20' : 'bg-white/[0.02] border-white/5'
                    )}>
                      <span className="text-xs font-bold text-muted-foreground w-5 text-center">{idx + 1}</span>
                      <div className="flex-1 min-w-0">
                        <p className={cn("text-sm font-medium truncate", isExtra ? 'text-amber-300' : 'text-foreground/90')}>
                          {catName}
                          {isExtra && <span className="ml-1.5 text-[10px] text-amber-400">⚡</span>}
                        </p>
                        <p className="text-[10px] text-muted-foreground mt-0.5">
                          {userMap.get(e.paidBy) ?? '?'} • {new Date(e.createdAt).toLocaleDateString('pt-BR')}
                        </p>
                      </div>
                      <span className={cn("text-sm font-bold flex-shrink-0", isExtra ? 'text-amber-400' : 'text-primary')}>
...
function SummaryCard({ label, value, icon, accent }: { label: string; value: string; icon: string; accent: string }) {
  const colors: Record<string, string> = {
    emerald: 'from-emerald-500/15 to-emerald-500/5 border-emerald-500/20 shadow-[0_0_20px_rgba(16,185,129,0.08)]',
    blue: 'from-blue-500/15 to-blue-500/5 border-blue-500/20 shadow-[0_0_20px_rgba(59,130,246,0.08)]',
    amber: 'from-amber-500/15 to-amber-500/5 border-amber-500/20 shadow-[0_0_20px_rgba(245,158,11,0.08)]',
    purple: 'from-purple-500/15 to-purple-500/5 border-purple-500/20 shadow-[0_0_20px_rgba(168,85,247,0.08)]',
  }
  ...
  return (
    <div className={cn("rounded-2xl bg-gradient-to-br border p-5 relative overflow-hidden transition-transform hover:scale-[1.02]", colors[accent])}>
